...
| 파일 | 역할 |
|------|------|
| `index.html` | 인트로(랜딩) 페이지 — 진입점. 단일 파일, 인라인 CSS |
...
# http://localhost:8001/ (인트로) → PLAY 버튼 → /play.html (게임)
```

또는 파일 탐색기에서 `index.html` / `play.html` 더블클릭으로 `file://` 실행.

## 기능 명세

### 게임 본문 (`play.html` + `script.js`)
...
- 그라데이션 타이틀 + 카피 + `PLAY →` 버튼 (`play.html` 로 진입)
